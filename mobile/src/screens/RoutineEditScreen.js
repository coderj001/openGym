...
export default function RoutineEditScreen({ navigation, route }) {
  ...
  const [picker, setPicker] = useState(false); const [selected, setSelected] = useState(null); const [index, setIndex] = useState(null);
  ...
  return <Screen><Header title={t('Edit routine')} left={<IconButton name="chevron-left" onPress={() => navigation.goBack()} />} /><Input value={routine.name} onChangeText={name => update(state => { state.routines.find(item => item.id === id).name = name; })} style={{ fontSize: 20, fontWeight: '800' }} />
...
const styles = StyleSheet.create({ chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, exercise: { paddingLeft: 4 }, actions: { flexDirection: 'row', gap: 4 }, smallButton: { width: 30, height: 30, borderRadius: 8 } });
