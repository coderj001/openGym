...
export default function HomeScreen({ navigation, route }) {
  const S = useStoreSelector(state => ({ routines: state.routines, activeName: state.active?.name || null, bodyweight: state.bodyweight, workouts: state.workouts, week: state.week, dayPlan: state.dayPlan, unit: state.unit, targetW: state.targetW, body: state.body, homeWidgets: state.homeWidgets })); const { update } = useStoreActions(); const colors = useColors();
  ...
  const today = todayISO(); const routine = effectiveRoutine(S, today); const bw = lastBW(S); const visibleWidgets = homeWidgetIds(S);
  ...
  const records = useMemo(() => visibleWidgets.includes('records') ? homeRecords(S) : [], [S.workouts, visibleWidgets.includes('records')]);
  const muscleLoad = useMemo(() => visibleWidgets.includes('muscles') ? loadOfWorkouts(S.workouts.slice(-12)) : {}, [S.workouts, visibleWidgets.includes('muscles')]); const muscleBalance = useMemo(() => rankOf(muscleLoad), [muscleLoad]); const maxMuscleLoad = muscleBalance.worked.length ? muscleLoad[muscleBalance.worked[0]] : 1;
  const recentWorkouts = useMemo(() => visibleWidgets.includes('recent') ? S.workouts.slice(-3).reverse() : [], [S.workouts, visibleWidgets.includes('recent')]);
  const volumePoints = useMemo(() => visibleWidgets.includes('volume') ? S.workouts.slice(-8).map(workout => ({ y: workoutVolume(workout) })) : [], [S.workouts, visibleWidgets.includes('volume')]);
  const nextWorkout = useMemo(() => visibleWidgets.includes('next') ? nextPlannedWorkout(S, today) : null, [S.routines, S.week, S.dayPlan, today, visibleWidgets.includes('next')]);
  const weightChange = useMemo(() => visibleWidgets.includes('weight-change') ? bodyWeightChange(S) : null, [S.bodyweight, visibleWidgets.includes('weight-change')]);
  const streak = useMemo(() => visibleWidgets.includes('streak') ? streakWeeks(S) : 0, [S.workouts, visibleWidgets.includes('streak')]);
  ...
  const widgetCards = {
    schedule: () => <Card><View style={styles.calHeader}><AppText style={{ fontWeight: '700' }}>{calView === 'week' ? t('This week') : calView === 'month' ? MONTH_NAMES[new Date().getMonth()] + ' ' + new Date().getFullYear() : String(new Date().getFullYear())}</AppText><View style={styles.calTabs}>{['week','month','year'].map(v => <Pressable key={v} onPress={() => setCalView(v)} style={[styles.calTab, calView === v && { backgroundColor: colors.accent }]}><AppText style={{ fontSize: 12, fontWeight: '700', color: calView === v ? (colors.dark ? '#fff' : '#000') : colors.muted }}>{v === 'week' ? t('W') : v === 'month' ? t('M') : t('Y')}</AppText></Pressable>)}</View></View>{calView === 'week' && <WeekView weekDays={weekDays} today={today} done={done} S={S} colors={colors} setOverrideDay={setOverrideDay} highlightDate={highlightDate} />}{calView === 'month' && <MonthView today={today} done={done} colors={colors} highlightDate={highlightDate} />}{calView === 'year' && <YearView today={today} done={done} colors={colors} highlightDate={highlightDate} />}<Pressable onPress={S.activeName ? () => navigation.navigate('Workout') : start} style={[styles.today, { borderTopColor: colors.border }]}><View style={[styles.bigIcon, { backgroundColor: S.activeName ? colors.orange : routine ? colors.accent : colors.surface2 }]}><Icon name={S.activeName ? 'timer-outline' : routine ? 'dumbbell' : 'weather-night'} size={23} color={routine || S.activeName ? '#fff' : colors.muted} /></View><View style={{ flex: 1 }}><AppText muted style={{ fontSize: 12 }}>{t('Today')}</AppText><AppText style={{ fontWeight: '800' }}>{S.activeName ? `${S.activeName} — ${t('in progress')}` : routine?.name || t('Rest day')}</AppText></View><AppText style={{ color: colors.accent, fontWeight: '800' }}>{S.activeName ? t('Resume') : routine ? t('Start') : '+'}</AppText></Pressable></Card>,
    ...
    streak: () => <Card><View style={styles.between}><View><AppText style={{ fontSize: 21, fontWeight: '800' }}>🔥 {t('{0} week streak', streak)}</AppText><AppText muted style={{ marginTop: 4 }}>{weekCount} {t('this week')} · {t('{0} workouts total', S.workouts.length)}</AppText></View><Icon name="calendar-month" size={28} color={colors.muted} /></View></Card>,
  ...
  const welcome = !S.routines.length && !S.activeName ? <Card><AppText style={{ fontSize: 22, fontWeight: '800' }}>{t('Welcome!')}</AppText><AppText muted style={{ lineHeight: 21, marginVertical: 8 }}>{t('Set up your weekly routine to get going — or load a ready-made Push / Pull / Legs plan.')}</AppText><Button title={t('Load starter plan (PPL)')} icon="creation" primary onPress={() => update(addStarterPlan)} /><View style={{ height: 8 }} /><Button title={t('Build my own plan')} onPress={() => navigation.navigate('Plan')} /></Card> : null;
  return <Screen>
    ...
    {visibleWidgets.map(id => <React.Fragment key={id}>{widgetCards[id]()}<React.Fragment>{id === 'schedule' ? welcome : null}</React.Fragment></React.Fragment>)}
