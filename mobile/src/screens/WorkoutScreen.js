...
function ActiveWorkout({ onFinished }) {
  ...
  const A = S.active; const units = supersetUnits(A.entries); const current = Math.min(A.cur, Math.max(0, A.entries.length - 1)); const unit = unitOf(units, current); const unitIndex = units.findIndex(item => item.includes(current)); const total = A.entries.reduce((sum, entry) => sum + entry.sets.length, 0); const done = setsDoneActive(A); const nextRestSet = units.slice(unitIndex).flatMap(restUnit => restUnit.flatMap(entryIndex => A.entries[entryIndex].sets.map((set, setIndex) => ({ entryIndex, setIndex, set })))).find(({ set }) => !set.done); const nextRestLabel = nextRestSet ? `${exOr(A.entries[nextRestSet.entryIndex].id).n} · ${t('Set {0}', nextRestSet.setIndex + 1)}/${A.entries[nextRestSet.entryIndex].sets.length}` : null;
