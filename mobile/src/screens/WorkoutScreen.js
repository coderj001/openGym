...
function ActiveWorkout({ onFinished }) {
  ...
  const toggle = useCallback((entryIndex, setIndex) => {
    ...
    if (!wasDone) { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light).catch(() => {}); if (S.sound) { player.seekTo(0); player.play(); } if (allWorkout) stopRest(); else if (!allEntry || lastInUnit) startRest(S.restSec); }
  }, [S.restSec, S.sound, player, startRest, stopRest, update]);
