...
export default function WorkoutScreen({ navigation, route }) {
  ...
  if (!active) {
    if (summary) return <FinishSummary workout={summary} onDone={() => { setSummary(null); navigation.navigate('Home'); }} />;
...
function ActiveWorkout({ onFinished }) {
  ...
  const copyPrevious = entryIndex => {
    ...
    const previous = previousPerformance(S, entry.id, entry.target, S.unit);
...
const EntryBlock = React.memo(function EntryBlock({ entryIndex, onToggle, mutate, onDetail, onWork, onSetMenu, onTray, onCopyPrevious }) {
  ...
  const previous = S.showPrevious ? previousPerformance(S, entry.id, entry.target, S.unit) : null;
...
const styles = StyleSheet.create({ summaryHero: { alignItems: 'center', gap: 6, paddingVertical: 20 }, summaryTitle: { fontSize: 24, fontWeight: '800' }, summaryGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, summaryTile: { width: '48%', gap: 4 }, summaryValue: { fontSize: 20, fontWeight: '800' }, summaryRow: { flexDirection: 'row', alignItems: 'center', gap: 7, marginTop: 8 }, previousCard: { padding: 10 }, copyUndo: { padding: 8 }, between: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10 }, elapsed: { flexDirection: 'row', justifyContent: 'space-between' }, nav: { flexDirection: 'row', gap: 8, marginTop: 6 }, setRow: { minHeight: 62, flexDirection: 'row', alignItems: 'center', gap: 6, borderBottomWidth: StyleSheet.hairlineWidth }, setNumberBtn: { width: 32, alignItems: 'center', justifyContent: 'center', paddingVertical: 4, borderRadius: 8 }, setField: { flex: 1, minWidth: 56, borderRadius: 9, alignItems: 'center', paddingVertical: 4 }, setInput: { minWidth: 48, textAlign: 'center', fontWeight: '800', fontSize: 17, padding: 0 }, check: { width: 38, height: 38 }, overlay: { flex: 1, backgroundColor: '#000b', justifyContent: 'center', padding: 22 }, dialog: { borderRadius: 20, padding: 18, gap: 12 }, qrDialog: { alignItems: 'center' }, qr: { padding: 12, backgroundColor: '#fff', borderRadius: 14 }, scanner: { flex: 1, padding: 16, gap: 12 }, camera: { flex: 1, minHeight: 360, borderRadius: 20, overflow: 'hidden' }, sharedEntry: { paddingVertical: 10, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#8886' } });
