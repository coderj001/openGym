...
export default function WorkoutScreen({ navigation, route }) {
  const S = useStoreSelector(state => ({ active: !!state.active, bodyweight: state.bodyweight, keepAwake: state.keepAwake })); const { update } = useStoreActions(); const active = S.active; const [summary, setSummary] = useState(null); const [startId, setStartId] = useState(undefined); const [startPlan, setStartPlan] = useState(null); const [weight, setWeight] = useState('');
...
function FinishSummary({ workout, onDone }) {
  const S = useStoreSelector(state => ({ unit: state.unit, body: state.body })); const colors = useColors(); const load = loadOfWorkouts([workout]); const muscles = new Set(Object.entries(load).filter(([, value]) => value > 0).map(([muscle]) => muscle)); const summary = advancedSetsSummary(workout);
...
export function nextIncompleteSet(entries, units, unitIndex) {
  for (let at = Math.max(0, unitIndex); at < units.length; at++) {
    for (const entryIndex of units[at]) {
      const setIndex = entries[entryIndex].sets.findIndex(set => !set.done);
      if (setIndex >= 0) return { entryIndex, setIndex };
    }
  }
  return null;
}
function ActiveWorkout({ onFinished }) {
  const S = useStoreSelector(state => ({ active: state.active, showPrevious: state.showPrevious, workouts: state.workouts, unit: state.unit, sound: state.sound, restTimer: state.restTimer, restSec: state.restSec, effort: state.effort, showRir: state.showRir })); const { update } = useStoreActions(); const colors = useColors(); const { startRest, stopRest } = useTimers(); const player = useSoundPlayer(require('../../assets/beep.wav')); const [picker, setPicker] = useState(false); const [copyUndo, setCopyUndo] = useState(null); const [configEx, setConfigEx] = useState(null); const [detail, setDetail] = useState(null); const [work, setWork] = useState(null); const [setMenu, setSetMenu] = useState(null); const [tray, setTray] = useState(null);
  useEffect(() => { if (S.restTimer === false) stopRest(); }, [S.restTimer, stopRest]);
  const A = S.active; const unitKey = A.entries.map(entry => entry.sg || '').join('|'); const units = useMemo(() => supersetUnits(A.entries), [A.entries.length, unitKey]); const current = Math.min(A.cur, Math.max(0, A.entries.length - 1)); const unit = unitOf(units, current); const unitIndex = units.findIndex(item => item.includes(current)); const total = A.entries.reduce((sum, entry) => sum + entry.sets.length, 0); const done = setsDoneActive(A); const nextRestSet = useMemo(() => nextIncompleteSet(A.entries, units, unitIndex), [A.entries, units, unitIndex]); const nextRestLabel = nextRestSet ? `${exOr(A.entries[nextRestSet.entryIndex].id).n} · ${t('Set {0}', nextRestSet.setIndex + 1)}/${A.entries[nextRestSet.entryIndex].sets.length}` : null;
  ...
  const mutateEntry = useCallback((entryIndex, fn) => update(state => fn(state.active.entries[entryIndex])), [update]);
  const copyPrevious = useCallback((entryIndex, previous) => {
    if (!previous?.compatible) return;
    let before = [];
    update(state => {
      const entry = state.active.entries[entryIndex];
      const fields = ['w', 'r', 'sec', 'min', 'speed'];
      before = entry.sets.map((set, index) => set.done || set.kind || set.fail ? null : { index, values: Object.fromEntries(fields.map(field => [field, set[field]])) }).filter(Boolean);
      entry.sets = copyPreviousSets(entry.sets, previous.sets, modeOf(entry.target));
    });
  ...
  const toggle = useCallback((entryIndex, setIndex) => {
    let wasDone = false; let allEntry = false; let allWorkout = false; let lastInUnit = false;
    update(state => {
      const active = state.active; const entry = active.entries[entryIndex]; const set = entry.sets[setIndex];
      wasDone = set.done; set.done = !set.done;
      allEntry = entry.sets.every(item => item.done);
      allWorkout = active.entries.every(item => item.sets.every(value => value.done));
      const activeUnit = unitOf(supersetUnits(active.entries), entryIndex);
      lastInUnit = activeUnit[activeUnit.length - 1] === entryIndex;
    });
    if (!wasDone) { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light).catch(() => {}); if (S.sound) { player.seekTo(0); player.play(); } if (allWorkout) stopRest(); else if (S.restTimer !== false && (!allEntry || lastInUnit)) startRest(S.restSec); }
  }, [S.restTimer, S.restSec, S.sound, player, startRest, stopRest, update]);
  const startWork = useCallback((entryIndex, setIndex, seconds) => setWork({ entryIndex, setIndex, seconds, started: Date.now() }), []);
  const openSetMenu = useCallback((entryIndex, setIndex, set) => setSetMenu({ entryIndex, setIndex, set }), []);
  const finish = () => { if (!done) return Alert.alert(t('Nothing logged'), t('Complete at least one set first.')); Alert.alert(t('Finish workout?'), t('{0} sets completed', done), [{ text: t('Cancel') }, { text: t('Finish'), onPress: () => { let workout; update(state => { const finished = finishActiveWorkout(state); workout = finished ? JSON.parse(JSON.stringify(finished)) : null; }); stopRest(); if (S.sound) { player.seekTo(0); player.play(); } if (workout) onFinished(workout); } }]); };
  ...
  return <Screen><Header title={A.name} subtitle={`${Math.floor(done)}/${total} ${t('sets')}`} left={<IconButton name="close" onPress={discard} />} right={<IconButton name="check" color={colors.accent} onPress={finish} />} /><View style={styles.elapsed}><Elapsed start={A.start} /><AppText muted>{unit.length > 1 ? t('Superset {0} / {1}', unitIndex + 1, units.length) : t('Exercise {0} / {1}', unitIndex + 1, units.length)}</AppText></View><Progress value={total ? done / total : 0} />
    ...
    {A.entries.length ? unit.map(entryIndex => <EntryBlock key={entryIndex} entryIndex={entryIndex} previous={previousByEntry[entryIndex]} onToggle={toggle} mutate={mutateEntry} onDetail={setDetail} onWork={startWork} onSetMenu={openSetMenu} onTray={setTray} onCopyPrevious={copyPrevious} />) : <Card><AppText muted style={{ textAlign: 'center' }}>{t('Freestyle workout — add your first exercise.')}</AppText></Card>}
...
const EntryBlock = React.memo(function EntryBlock({ entryIndex, previous, onToggle, mutate, onDetail, onWork, onSetMenu, onTray, onCopyPrevious }) {
  const selected = useStoreSelector(state => ({ entry: state.active?.entries[entryIndex], unit: state.unit, effort: state.effort, showRir: state.showRir })); const colors = useColors(); const entry = selected.entry; if (!entry) return null; const exercise = exOr(entry.id); const mode = modeOf({ ...entry.target, id: entry.id }); const effort = EFFORT[effortOf(selected)]; const bodyweight = isBw({ ...entry.target, id: entry.id });
  const change = (setIndex, key, value) => mutate(entryIndex, item => { item.sets[setIndex][key] = value === undefined ? undefined : Math.max(0, Math.round(value * 10) / 10); });
  const addSet = () => mutate(entryIndex, item => { const last = item.sets[item.sets.length - 1] || {}; item.sets.push({ ...last, done: false }); }); const remove = () => mutate(entryIndex, item => { if (item.sets.length > 1) item.sets.pop(); });
  return <View style={{ gap: 10 }}><ExerciseMedia exercise={exercise} compact /><View style={styles.between}><Pressable onPress={() => onDetail(exercise)} style={{ flex: 1 }}><AppText style={{ fontSize: 21, fontWeight: '800', textTransform: 'capitalize' }}>{exercise.n}</AppText><AppText muted style={{ textTransform: 'capitalize' }}>{exercise.tg || exercise.bp} · {exercise.eq}</AppText></Pressable><IconButton name="information-outline" onPress={() => onDetail(exercise)} /></View>{previous ? <Card style={styles.previousCard}><View style={styles.between}><AppText muted style={{ fontSize: 12 }}>{t('Last session')} · {fmtDate(previous.d, true)}</AppText>{previous.compatible ? <Button compact title={t('Use last values')} onPress={() => onCopyPrevious(entryIndex, previous)} /> : null}</View><AppText muted numberOfLines={2} style={{ marginTop: 6 }}>{previous.labels.join('  ·  ')}</AppText>{!previous.compatible ? <AppText dim style={{ fontSize: 12, marginTop: 4 }}>{t('Different exercise mode — values cannot be copied.')}</AppText> : null}</Card> : null}{entry.plan?.why && !['off', 'first'].includes(entry.plan.kind) ? <Card style={{ backgroundColor: `${colors.accent}18` }}><AppText style={{ color: colors.accent, fontSize: 13 }}>{t(...entry.plan.why)}</AppText></Card> : null}<Card style={{ paddingVertical: 8 }}>{entry.sets.map((set, setIndex) => <View key={setIndex} style={[styles.setRow, { borderBottomColor: colors.border, opacity: set.done ? .55 : 1 }]}><Pressable onPress={() => onSetMenu(entryIndex, setIndex, set)} style={[styles.setNumberBtn, { backgroundColor: colors.surface2 }]} accessibilityLabel={t('Set options')}><AppText style={{ fontWeight: '800', color: set.fail ? colors.danger : colors.text, fontSize: 15 }}>{set.kind === 'w' ? 'W' : set.kind === 'd' ? 'D' : set.kind === 'a' ? 'A' : setIndex + 1}</AppText><Icon name="dots-horizontal" size={12} color={colors.dim} style={{ marginTop: -2 }} /></Pressable>{mode === 'cardio' ? <><SetField onTray={onTray} type="time" step={5} value={set.min} suffix="min" onChange={value => change(setIndex, 'min', value)} /><SetField onTray={onTray} type="speed" step={1} value={set.speed} suffix="km/h" onChange={value => change(setIndex, 'speed', value)} /></> : mode === 'time' ? <><SetField onTray={onTray} type="time" step={5} value={set.sec} suffix="s" onChange={value => change(setIndex, 'sec', value)} />{!bodyweight || set.w > 0 ? <SetField onTray={onTray} type="weight" step={2.5} value={set.w} suffix={selected.unit} onChange={value => change(setIndex, 'w', value)} /> : null}<IconButton name="play" disabled={set.done} onPress={() => onWork(entryIndex, setIndex, set.sec || 45)} style={styles.check} /></> : <>{!bodyweight || set.w > 0 ? <SetField onTray={onTray} type="weight" step={2.5} value={set.w} suffix={selected.unit} onChange={value => change(setIndex, 'w', value)} /> : null}<SetField onTray={onTray} type="reps" step={1} value={set.r} suffix={set.kind === 'a' ? 'reps (AMRAP)' : 'reps'} onChange={value => change(setIndex, 'r', value)} placeholder={set.kind === 'a' || set.kind === 'd' ? '?' : undefined} />{effort ? <SetField onTray={onTray} type="effort" step={1} value={set[effort.f] ?? ''} suffix={effort.hd} onChange={value => change(setIndex, effort.f, Math.min(effort.max, value))} /> : null}</>}<IconButton name={set.done ? 'check-circle' : 'checkbox-blank-circle-outline'} color={set.done ? colors.accent : colors.muted} onPress={() => onToggle(entryIndex, setIndex)} style={styles.check} /></View>)}<View style={styles.nav}><Button compact title={t('Remove set')} disabled={entry.sets.length <= 1} onPress={remove} style={{ flex: 1 }} /><Button compact title={t('Add set')} icon="plus" onPress={addSet} style={{ flex: 1 }} /></View></Card></View>;
});
