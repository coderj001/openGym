...
export default function PlanScreen({ navigation }) {
  ...
  return <Screen><Header title={t('Plan')} subtitle={t('Your weekly routine')} right={<IconButton name="plus" onPress={add} />} /><SectionTitle>{t('Week schedule')}</SectionTitle><Card style={{ paddingVertical: 0 }}>{[1, 2, 3, 4, 5, 6, 0].map(value => { const routine = S.routines.find(item => item.id === S.week[value]); return <Row key={value} icon="calendar" title={t(DAYN[value])} onPress={() => setDay(value)}>{routine ? <View style={[styles.tag, { backgroundColor: `${colors.accent}25` }]}><Icon name={routine.emoji || 'dumbbell'} size={16} color={colors.accent} /><AppText style={{ color: colors.accent, fontSize: 13, fontWeight: '700' }}>{routine.name}</AppText></View> : <AppText muted>{t('Rest')}</AppText>}</Row>; })}</Card>
    ...
    {S.routines.length ? <Card style={{ paddingVertical: 0 }}>{S.routines.map(routine => <Row key={routine.id} icon={routine.emoji || 'dumbbell'} title={routine.name} subtitle={exCount(routine.ex.length)} onPress={() => navigation.navigate('RoutineEdit', { id: routine.id })} />)}</Card> : <Card><AppText muted style={{ textAlign: 'center', marginBottom: 12 }}>{t('No routines yet. Create one or load the starter plan.')}</AppText><Button title={t('Load starter plan (Push / Pull / Legs)')} icon="creation" primary onPress={() => update(addStarterPlan)} /></Card>}
    <Modal transparent visible={day !== null} animationType="fade" onRequestClose={() => setDay(null)}><View style={styles.overlay}><View style={[styles.dialog, { backgroundColor: colors.surface }]}><AppText style={{ fontSize: 22, fontWeight: '800' }}>{day !== null ? t(DAYN[day]) : ''}</AppText><Pressable style={styles.choice} onPress={() => { update(state => { delete state.week[day]; }); setDay(null); }}><Icon name="weather-night" size={22} color={colors.muted} /><AppText>{t('Rest')}</AppText></Pressable>{S.routines.map(routine => <Pressable key={routine.id} style={styles.choice} onPress={() => { update(state => { state.week[day] = routine.id; }); setDay(null); }}><Icon name="dumbbell" size={22} color={colors.accent} /><AppText>{routine.name}</AppText></Pressable>)}<Button title={t('Cancel')} onPress={() => setDay(null)} /></View></View></Modal>
  </Screen>;
}
const styles = StyleSheet.create({ tag: { flexDirection: 'row', alignItems: 'center', gap: 5, borderRadius: 12, paddingHorizontal: 9, paddingVertical: 5 }, heading: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }, overlay: { flex: 1, justifyContent: 'center', padding: 22, backgroundColor: '#000a' }, dialog: { padding: 18, borderRadius: 18, gap: 8 }, choice: { minHeight: 48, flexDirection: 'row', alignItems: 'center', gap: 10 } });
