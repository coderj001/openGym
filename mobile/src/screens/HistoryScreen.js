...
export default function HistoryScreen({ navigation, route }) {
  ...
  return <Screen scroll={false} contentStyle={{ paddingBottom: 0 }}><Header title={t('History')} subtitle={t('{0} workouts', S.workouts.length)} left={<IconButton name="chevron-left" onPress={() => navigation.goBack()} />} /><FlatList data={workouts} keyExtractor={workout => workout.id} initialNumToRender={15} contentContainerStyle={styles.list} ListEmptyComponent={<Card><AppText muted style={{ textAlign: 'center' }}>{t('No workouts yet.')}</AppText></Card>} renderItem={({ item: workout }) => { const summary = advancedSetsSummary(workout); return <Pressable onPress={() => setSelected(workout)}><Card><View style={styles.between}><View style={{ flex: 1 }}><AppText style={{ fontWeight: '800', fontSize: 17 }}>{workout.name}</AppText><AppText muted style={{ marginTop: 3 }}>{fmtDate(workout.d, true)} · {fmtDur(workout.duration || workout.end - workout.start)} · {summary.total} {t('sets')}{summary.text ? ` (${summary.text})` : ''}</AppText></View><AppText style={{ color: colors.accent, fontWeight: '800' }}>{fmtNum(workoutVolume(workout))} {S.unit}</AppText></View></Card></Pressable>; }} />
...
const styles = StyleSheet.create({ list: { gap: 12, paddingBottom: 120 }, between: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 10 } });
