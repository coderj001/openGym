...
const SHOW_DEMO_DATA = __DEV__ && process.env.EXPO_PUBLIC_SHOW_DEMO_DATA === 'true';
...
export default function SettingsScreen({ navigation }) {
  ...
  return <Screen><Header title={t('Settings')} left={<IconButton name="chevron-left" onPress={() => navigation.goBack()} />} />{message ? <Card style={{ backgroundColor: `${colors.accent}22` }}><AppText style={{ color: colors.accent, textAlign: 'center', fontWeight: '700' }}>{message}</AppText></Card> : null}
    ...
    <SectionTitle>{t('During a workout')}</SectionTitle><Card><Row icon="timer-outline" title={t('Rest timer')}><Toggle value={S.restTimer !== false} onValueChange={value => update(state => { state.restTimer = value; })} /></Row>{S.restTimer !== false ? <View style={styles.chips}>{[60, 90, 120, 150, 180].map(value => <Chip key={value} title={`${value}s`} active={S.restSec === value} onPress={() => update(state => { state.restSec = value; })} />)}</View> : null}<Row icon="weather-sunny" title={t('Keep screen awake')}><Toggle value={S.keepAwake !== false} onValueChange={value => update(state => { state.keepAwake = value; })} /></Row><Row icon="history" title={t('Show previous performance')} subtitle={t('Show the last completed sets beside each exercise.')}><Toggle value={S.showPrevious === true} onValueChange={value => update(state => { state.showPrevious = value; })} /></Row><Row icon="bell" title={t('Sounds')}><Toggle value={!!S.sound} onValueChange={value => update(state => { state.sound = value; })} /></Row><AppText muted style={styles.label}>{t('Effort per set')}</AppText><View style={styles.chips}>{['none', 'rir', 'rpe'].map(value => <Chip key={value} title={value === 'none' ? t('Off') : value.toUpperCase()} active={effortOf(S) === value} onPress={() => update(state => { state.effort = value; delete state.showRir; })} />)}</View></Card>
    ...
    <AppText dim style={{ textAlign: 'center', marginTop: 12, lineHeight: 20, paddingBottom: 24 }}>
      ...
      <AppText accessibilityRole="link" accessibilityLabel={t('Report an issue on GitHub')} onPress={() => Linking.openURL('https://github.com/coderj001/openGym/issues/new/choose').catch(() => notify(t('Could not open GitHub')))} style={{ textDecorationLine: 'underline', color: colors.accent }}>
