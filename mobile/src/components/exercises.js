...
export function ExerciseMedia({ exercise, compact = false, mini = false }) {
  const source = mediaFor(exercise); const colors = useColors(); const height = mini ? 112 : compact ? 180 : 280; const player = useRef(null); const [playingGif, setPlayingGif] = useState(null); const playing = playingGif === exercise?.gif;
  ...
  const toggle = () => { if (playing) player.current?.stopAnimating(); else player.current?.startAnimating(); setPlayingGif(playing ? null : exercise.gif); };
  return <Pressable accessibilityRole="button" accessibilityLabel={t(playing ? 'Pause exercise animation' : 'Play exercise animation')} onPress={toggle} style={[styles.media, { height }]}><Image ref={player} source={source} style={[styles.mediaImage, { backgroundColor: '#fff' }]} contentFit="contain" autoplay={playing} />{!playing ? <View pointerEvents="none" testID="exercise-media-play" style={styles.mediaOverlay}><View style={styles.mediaControl}><Icon name="play" size={26} color="#fff" /></View></View> : null}</Pressable>;
}
