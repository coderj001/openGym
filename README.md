...
`mobile/` is a standalone **Expo and React Native** app for Android and iOS. It has no login,
backend, synchronization, or runtime network dependency: workouts, routines, history, body
weight, preferences, and reminders stay on the device. All 1,324 exercise animations and
translated instruction packs are included in the app.

### Prerequisites

- Node.js 20 or newer
- npm
- For Android emulators: Android Studio
- For iOS simulators: macOS with Xcode
...
npm run android    # open on an Android emulator or connected device
npm run ios        # open on an iOS simulator or connected device (macOS only)
npm run web        # optional browser preview
```

The mobile app does not require Docker, `.env`, the API, or a self-hosted openGym instance.
After installation, its workout and exercise features work offline.

To create local native projects and development builds instead of using Expo Go:
...
Native projects generated by these commands stay under `mobile/android` and `mobile/ios` and
are intentionally ignored by Git. See [mobile/README.md](mobile/README.md) for local-data,
backup, import, and notification instructions.
